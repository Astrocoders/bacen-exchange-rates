...
module ExchangeRateDecode = {
  type exchangeReport =
    ...
    | Intermediary
    | PTAXClosing;
  let exchangeReportFromString = value =>
    switch (value) {
    | "Abertura" => Opening
  ...
  let entry = json => {
    open! Json.Decode;
    {
      ...
      report:
        json |> field("tipoBoletim", string) |> exchangeReportFromString,
    };
  };
  let decode = json => Json.Decode.(json |> field("value", array(entry)));
};

let getFetchURL = (~currency: currency, ~date) => {
  let currencyStr = currencyToJs(currency);
  let formattedDate =
    MomentRe.(date |> momentWithDate |> Moment.format("MM-DD-YYYY"));
